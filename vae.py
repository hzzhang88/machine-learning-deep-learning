...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.encoder = torch.nn.Sequential(torch.nn.Flatten(1,3),
                                        torch.nn.Linear(input_size,self.hidden_dim),
                                        torch.nn.ReLU(),
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        self.decoder = torch.nn.Sequential(torch.nn.Linear(self.latent_size,self.hidden_dim),
                                            torch.nn.ReLU(),
                                            torch.nn.Linear(self.hidden_dim,self.hidden_dim),
                                            torch.nn.ReLU(),
                                            torch.nn.Linear(self.hidden_dim,self.hidden_dim),
                                            ...
                                            torch.nn.Sigmoid(),
                                            torch.nn.Unflatten(-1,unflattened_size = (1,28,28))
                                            )
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        self.encoder = torch.nn.Sequential(
                                        torch.nn.Linear(input_size+self.num_classes,self.hidden_dim),
                                        torch.nn.ReLU(),
                                        torch.nn.Linear(self.hidden_dim,self.hidden_dim),
                                        torch.nn.ReLU(),
                                        torch.nn.Linear(self.hidden_dim,self.hidden_dim),
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        self.decoder = torch.nn.Sequential(torch.nn.Linear(self.latent_size+self.num_classes,self.hidden_dim),
                                            torch.nn.ReLU(),
                                            torch.nn.Linear(self.hidden_dim,self.hidden_dim),
                                            torch.nn.ReLU(),
                                            torch.nn.Linear(self.hidden_dim,self.hidden_dim),
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        xx = torch.flatten(x,1,3).clone()
        temp = self.encoder(torch.cat([xx,c],dim = 1))
        mu = self.mu_layer(temp)
        logvar = self.logvar_layer(temp)
        z = reparametrize(mu,logvar)
...
def reparametrize(mu, logvar):
    ...
    # Replace "pass" statement with your code
    N,Z = mu.shape
    e = torch.randn(N,Z,device = mu.device)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # Replace "pass" statement with your code
    N,Z = mu.shape
    reconstruction = F.binary_cross_entropy(x_hat,x,reduction = 'sum')/N
    KL= -0.5*(1.+logvar-mu**2-torch.exp(logvar)).sum()/N
    loss = KL +reconstruction
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
