...
def prepocess_input_sequence(
    ...
    for s in string:
        if s in spc_tokens:
            out.append(token_dict[s])
        else:
            digits = list(s)
            for d in digits:
                out.append(token_dict[d])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    import  math
    K,M = query.shape
    E = query.mm(key.T).div(math.sqrt(M))
...
def scaled_dot_product_two_loop_batch(
    ...
    import  math
    # N,K,M X (N,M,K) ---> (N,K,K)
    E = query.bmm(key.permute(0,2,1)).div(math.sqrt(M))
    # (N,K,K) ----> (N,K,k)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        torch.nn.init.uniform_(self.q.weight, -c, c)
        self.k = nn.Linear(dim_in,dim_q)
        torch.nn.init.uniform_(self.k.weight, -c, c)

        c_prime = math.sqrt(6./(dim_in + dim_v))  
        self.v = nn.Linear(dim_in,dim_v)
        torch.nn.init.uniform_(self.v.weight, -c_prime , c_prime )
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
#         torch.nn.init.uniform_(self.linear.weight, -c , c )
...
#         # N,K
#         sigma = torch.sqrt((center**2).sum(-1)/self.emb_dim +self.epsilon)
#         # N,K,M
#         z= center/sigma.unsqueeze(-1)

#         # 
#         y =  torch.mul(z,self.gamma)+ self.beta
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        x = self.dropout(self.norm1(x + self.MultiHeadBlock(x,x,x)))
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    # N,K= seq.shape
    # mask = torch.ones(K,K,device=seq.device).triu().fill_diagonal_(0)
    # mask = mask.unsqueeze(0).repeat(N,1,1)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    if M%2 ==0:
        A = torch.div(torch.arange(0,M,step = 2,dtype = torch.float32),M,rounding_mode ='floor')
        for k in range(K):
            y[k,0::2] = torch.sin(k/(10000.**A))
            y[k,1::2] =torch.cos(k/(10000.**A))
    else:
        A = torch.div(torch.arange(0,M,step = 2,dtype = torch.float32),M,rounding_mode ='floor')
        for k in range(K):
            y[k,0::2] = torch.sin(k/(10000.**A))
            y[k,1::2] =torch.cos(k/(10000.**A[:-1]))
